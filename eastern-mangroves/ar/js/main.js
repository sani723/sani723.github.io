	...
	homeParallax:function(el,speed){
		...
		if($(window).width()>970) {
			$(el).each(function(){
				...
				$self.css({'backgroundImage':"url(/ar/"+$bg.attr('src')+")",'backgroundAttachment':'fixed','backgroundPosition':'50% 130px','backgroundRepeat':'no-repeat'});
				
				$(window).bind('scroll',function(){
					if(parseInt($(this).scrollTop())>=(topOffset) && ((topOffset)+$self.height()>$(this).scrollTop())){
						$steppers.removeClass('active');
						$steppers.eq(current).addClass('active');	
					}
					if(parseInt($(this).scrollTop()+($(this).height()))>=(topOffset) && ((topOffset)+$self.height()>$(this).scrollTop())){
